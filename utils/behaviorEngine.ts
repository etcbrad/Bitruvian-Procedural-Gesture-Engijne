...
export class BehaviorEngine {
  ...
  static blendPose(
    ...
    const blended: Partial<WalkingEnginePose> = {};
    ...
    keys.forEach((key) => {
      ...
      const lVal = locPose[k] ?? 0;
      const iVal = idlePose[k] ?? 0;
      const sVal = scriptPose[k] ?? 0;
      ...
      if (k === 'x_offset' || k === 'y_offset') {
        blended[k] = base;
      } else {
        blended[k] = base + (sVal * activitySuppression);
      }
    });
